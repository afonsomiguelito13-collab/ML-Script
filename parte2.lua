--[[ part2.lua — Fluent UI | Userspin45 ]]

local U = getgenv().Userspin45
if not U then warn("[Userspin45] part1 não carregou!") return end
...
-- MAIN
Tabs.Main:AddToggle("AS", {Title = "Auto Strength", Default = false, Callback = function(v)
    Config.AutoStrength = v
    if v then U.setAnim(Config.AnimSpeed) else U.setAnim(1) end
end})
Tabs.Main:AddToggle("FP", {Title = "Fast Punch", Default = false, Callback = function(v) Config.FastPunch = v end})
Tabs.Main:AddSlider("ASPD", {Title = "Anim Speed", Default = 1.5, Min = 1, Max = 3.5, Rounding = 1, Callback = function(v)
    Config.AnimSpeed = v
    if Config.AutoStrength or Config.FastPunch or Config.SilentKill or Config.SilentKill2 then U.setAnim(v) end
end})
...
Tabs.Kill:AddToggle("SK", {Title = "Silent Kill (Recomendado)", Default = false, Callback = function(v)
    Config.SilentKill = v
    if v then
...
Tabs.Kill:AddToggle("SK2", {Title = "Silent Kill 2 (Odiado)", Default = false, Callback = function(v)
...
Tabs.Kill:AddToggle("KA", {Title = "Kill Aura", Default = false, Callback = function(v)
    Config.KillAura = v
    if v then U.equipPunch() end
end})
Tabs.Kill:AddSlider("ASize", {Title = "Aura Size", Default = 12, Min = 5, Max = 40, Rounding = 0, Callback = function(v) Config.AuraSize = v end})
Tabs.Kill:AddToggle("ASH", {Title = "Auto Server Hop", Default = true, Callback = function(v) Config.AutoServerHop = v end})
...
Tabs.Kill:AddButton({Title = "Force Hop", Callback = U.hop})
...
for _, t in ipairs({
    ...
    Tabs.TP:AddButton({Title = t.n, Callback = function()
        U.refresh()
...
Tabs.Misc:AddSlider("WS", {Title = "WalkSpeed", Default = 16, Min = 16, Max = 200, Rounding = 0, Callback = function(v)
    U.refresh()
...
Tabs.Misc:AddButton({Title = "Salvar no AutoExecute", Callback = U.saveAuto})

-- CREDITS
Tabs.Credits:AddParagraph({Title = "Creator", Content = "👑 Userspin45\nMEGA FINAL + AutoExecute"})
Tabs.Credits:AddButton({Title = "Copiar Credits", Callback = function()
    setclipboard("Creator: Userspin45 | ML MEGA FINAL")
end})

print("[Userspin45] part2 OK")
